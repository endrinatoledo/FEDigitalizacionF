...
const TableList = () => {
  ...
  const onSync = async () =>{
    setLoading(true);
    await axios.get('/api/v1/syncClientData/')
    fillTable();
    message.success("Sincronización completada")
    setLoading(false);
    }
  const fillTable = async () => {
    let result = (await axios.get("/api/v1/partners") ).data;
    result = ( result == "Not Data" ? [] : result.filter( fil => fil.invoices.length == 1 ? String(fil.invoices[0].clients.cliId) === localStorage.getItem("client") : false).map((el, i) => {
        return {
          ...el, key: i
        }
      }))
  ...
  return (
    <ConfigProvider locale={esp}>
      <Button loading={loading} onClick={() => onSync()} type="primary" style={{ float: "left", marginTop: "20px" }} size="large" icon={<CloudSyncOutlined />}>
